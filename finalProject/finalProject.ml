      ...
      match l1 with
        ...
        | x :: remainder -> match x with
          | Valtuple (el1, el2) ->
              ...
              let freeVarEl1 = free_vars el1 in
              let del = delete l2 freeVarEl1 in
              let union2 = union l3 del in
      ...
      let rec aux l1 l2 l3 = match l1 with
        ...
        | x :: remainder -> match x with
          ...
          | Val(var1, var2) | ByName (var1, var2) ->
              ...
              let delftn = (delSetFunction (freeVars1) (l2)) in
              let appendDelFtn = (delftn) @ [var2] in
              aux remainder (appendDelFtn) (appendL3) in
              let (u1, u2) = aux l [] [] in
...
let rec subst ((e', x) : exp * name) (e : exp) : exp =
  ...
  | Let (ds, e2) ->
      ...
      let rec aux2 list1 list2 repl tr = (
        match list1 with
        | [] ->  (list2, repl, tr)
        | head :: remainder ->
          let ByName (y, n) = head in
...
let replfunct l1 var =
  let rec aux l2 var2 = match l2 with
    | x :: remainder ->
        let (l1, l2) = x in
        let substl = subst (l1, l2) var2 in
        aux remainder substl
    | [] -> var2
  in
  aux (List.rev l1) var
      ...
      | Apply (e1, e2) ->
          ...
          | Let (ds, e) ->
              let rec aux el1 el2 = match el1 with
                ...
                | x :: remainder -> match x with
                  | Val (var1, var2)  ->
                      let replace = replfunct el2 var1 in
                      let evalReplace = [eval (replfunct el2 var1), var2] in
                      ...
                      | Tuple (l1) ->
                          ...
                          let rec substTuples l1 l2 repl = match (l1, l2) with
                            | (x_l1::remainderL1, y_l2::remainderL2) ->
                                let replace = replfunct repl y_l2 in
...
let rec infer (ctx : context) (e : exp) : typ =  match e with
  ...
  | Let (l,el1) ->
        ...
        | x :: remainder -> (
            ...
            | Val (el1, el2) | ByName (el1, el2) -> (
                if aux list2 el2 then (
                  ...
                  let replaceEl1 = replfunct repl el1 in
                  ...
                  let replace = replfunct repl el1 in
            ...
            | Valtuple (l1, l2) -> (
                let replace = replfunct repl l1 in
                let matching = infer list2 replace in
                match matching with
                | TProduct (l3) ->
                    let rec aux2 l1x l2x l3x rep =
                      let tupLists = (l1x, l2x) in
                           match tupLists with
                           | ([], _) -> (l3x, rep)
      ...
      let (list2', repl2) = fold_ex l ctx [] in
      let replaceRepl2 = replfunct repl2 el1 in
